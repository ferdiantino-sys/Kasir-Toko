/**
 * app.js - Logika Utama Aplikasi POS Kasir Kelontong (Tailwind Edition)
 */
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function updateClock() {
        ...
        liveClock.textContent = `${now.toLocaleDateString("id-ID", options)} | ${timeStr}`;
    }
    ...
    function switchTab(tabId) {
        ...
        if (tabId === "pos") renderCatalog();
        else if (tabId === "inventory") renderInventoryTable();
        else if (tabId === "transactions") renderReportsAndHistory();
        else if (tabId === "settings") loadSettingsForm();
    }
    ...
    function renderCatalog() {
        ...
        filtered.forEach(p => {
            ...
            let catColorClass = "bg-emerald-500/90";
            if (remainingStock <= 5) catColorClass = "bg-rose-500/90";
    ...
    function addToCart(product) {
        ...
        if (cartItem) {
            if (cartItem.qty < maxStock) cartItem.qty++;
            else alert("Stok barang di toko tidak mencukupi.");
        } else {
            if (maxStock > 0) {
                cart.push({ id: product.id, name: product.name, price: product.price, costPrice: product.costPrice, qty: 1, discountAmount: 0 });
            }
        }
        renderCart();
        renderCatalog();
    ...
    btnViewCartMobile.addEventListener("click", () => {
        cartAreaElement.classList.remove("translate-y-full");
    });
    closeCartMobile.addEventListener("click", () => {
        cartAreaElement.classList.add("translate-y-full");
    });
    ...
    function renderCart() {
        ...
        if (cart.length === 0) {
            ...
            checkoutBtnMobile.classList.add("opacity-50", "pointer-events-none");
            cartSummaryWrapper.classList.add("hidden");
        } else {
            cartSummaryWrapper.classList.remove("hidden");
            checkoutBtn.classList.remove("opacity-50", "pointer-events-none");
            checkoutBtnMobile.classList.remove("opacity-50", "pointer-events-none");
            ...
            cart.forEach(item => {
                ...
                row.querySelector(".btn-discount").addEventListener("click", () => {
                    ...
                    if (discountStr !== null) {
                        ...
                        if (discountVal >= 0 && discountVal <= item.price) {
    ...
    document.addEventListener("keydown", (e) => {
        if (e.key === "F2") { e.preventDefault(); searchProductInput.focus(); }
        else if (e.key === "F8") { e.preventDefault(); if (cart.length > 0 && currentTab === "pos") openCheckout(); }
        else if (e.key === "Escape") { closeCheckout(); closeProduct(); }
    });
    ...
    function openCheckout() {
        ...
        btnConfirmCheckout.classList.add("opacity-50");
        ...
        radio.addEventListener("change", () => {
            ...
            paymentMethodCards.forEach(card => {
                if (card.querySelector("input").value === method) {
                    ...
                    card.classList.add("border-2", "text-emerald-600");
                    card.classList.remove("text-slate-500", "border");
                } else {
                    card.classList.replace("border-emerald-500", "border-outline-variant");
                    card.classList.remove("border-2", "text-emerald-600");
                    card.classList.add("text-slate-500", "border");
                }
            });
    ...
    function enableCheckoutBtn() {
        ...
        btnConfirmCheckout.classList.remove("opacity-50");
    }
    ...
    function generateQuickCashOptions() {
        ...
        exactBtn.addEventListener("click", () => { cashReceivedInput.value = bill; validateCashAmount(); });
        ...
        notes.forEach(note => {
            if (note >= bill) {
                ...
                btn.addEventListener("click", () => { cashReceivedInput.value = note; validateCashAmount(); });
    ...
    function validateCashAmount() {
        ...
        if (amountReceived >= calculatedTotal) {
            cashChangeDisplay.textContent = formatRupiah(amountReceived - calculatedTotal);
            enableCheckoutBtn();
        } else {
    ...
    btnConfirmCheckout.addEventListener("click", () => {
        ...
        const transaction = {
            ...
            total: calculatedTotal, paymentMethod: paymentMethod,
            payAmount: payAmount, changeAmount: Math.max(0, payAmount - calculatedTotal),
        ...
        if (dbResult.success) {
            ...
            closeCheckout();
            cart = []; summaryDiscountInput.value = "0";
            renderCart();
    ...
    function updateThermalReceiptTemplate(tx) {
        ...
        tx.items.forEach(item => {
            const p = item.price - item.discount;
    ...
    function renderInventoryTable() {
        ...
        if (filtered.length === 0) {
            inventoryList.innerHTML = `<div class="text-center p-md text-slate-500 font-label-sm">Belum ada produk.</div>`;
            return;
        }

        filtered.forEach(p => {
            const profit = p.price - p.costPrice;
            const margin = p.costPrice > 0 ? Math.round((profit / p.price) * 100) : 100;
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start mb-sm">
                    <div class="flex-1">
                        <span class="font-label-mono text-[11px] text-on-surface-variant">SKU: ${p.id}</span>
                        <h3 class="font-headline-md text-[16px] text-navy-900 mt-1">${p.name}</h3>
                        <span class="text-label-sm text-slate-400">Kat: ${p.category}</span>
                    </div>
                    <div class="flex gap-xs">
                        <button class="btn-edit w-10 h-10 flex items-center justify-center text-slate-600 bg-slate-100 rounded-full active:scale-95 transition-colors" data-id="${p.id}"><span class="material-symbols-outlined text-[18px]">edit</span></button>
                        <button class="btn-delete w-10 h-10 flex items-center justify-center text-rose-500 bg-rose-50 rounded-full active:scale-95 transition-colors" data-id="${p.id}"><span class="material-symbols-outlined text-[18px]">delete</span></button>
                    </div>
                </div>
                <div class="flex justify-between items-center mt-sm border-t border-slate-100 pt-sm">
                    <div class="flex flex-col">
                        <span class="font-label-sm text-[10px] text-on-surface-variant uppercase tracking-wider">Stok</span>
                        <div class="flex items-center gap-xs">
                            <span class="font-price-display text-[18px] ${isLowStock ? 'text-rose-500' : 'text-navy-900'}">${p.stock}</span>
                            <span class="font-body-md text-on-surface-variant text-[12px]">pcs</span>
                        </div>
                    </div>
                    <div class="text-right">
                        <span class="font-label-sm text-[10px] text-on-surface-variant uppercase tracking-wider">Harga Jual</span>
                        <p class="font-price-display text-[16px] text-emerald-600">${formatRupiah(p.price)}</p>
                    </div>
                </div>
            `;
    ...
    function openProductModal(productId = null) {
        ...
        if (productId) {
            ...
            if (prod) {
                prodIdInput.value = prod.id; prodIdInput.disabled = true;
    ...
    productForm.addEventListener("submit", (e) => {
        e.preventDefault();
        const product = {
            id: prodIdInput.value.trim(), barcode: prodBarcodeInput.value.trim(),
            name: prodNameInput.value.trim(), category: prodCategoryInput.value,
            stock: parseInt(prodStockInput.value) || 0, costPrice: parseInt(prodCostInput.value) || 0,
            price: parseInt(prodPriceInput.value) || 0
        };
        const result = editingProductId ? AppDB.updateProduct(product) : AppDB.addProduct(product);
    ...
    function renderReportsAndHistory() {
        ...
        transactions.forEach(tx => {
            ...
            let txProfit = 0;
            tx.items.forEach(item => { txProfit += ((item.price - item.costPrice - item.discount) * item.qty); });
        ...
        if (transactions.length === 0) {
            historyList.innerHTML = `<div class="text-center p-md text-slate-500 font-label-sm">Belum ada riwayat transaksi.</div>`;
            return;
        }
        ...
        transactions.slice().reverse().forEach(tx => {
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start">
                    <div class="flex flex-col">
                        <span class="font-label-mono text-[14px] text-navy-900 uppercase">#${tx.id}</span>
                        <span class="font-label-sm text-[11px] text-slate-500">${timeStr}</span>
                    </div>
                    <div class="flex flex-col items-end">
                        <span class="font-label-mono text-[16px] text-emerald-600">${formatRupiah(tx.total)}</span>
                        <span class="text-[10px] font-bold px-sm rounded mt-1 uppercase tracking-wider ${methodClass}">${tx.paymentMethod}</span>
                    </div>
                </div>
                <div class="flex gap-sm mt-2">
                    <button class="reprint-btn flex-[2] h-10 bg-slate-100 text-slate-700 font-label-sm rounded-lg active:scale-95 transition-transform flex items-center justify-center gap-xs" data-id="${tx.id}">
                        <span class="material-symbols-outlined text-[18px]">print</span> Cetak Struk
                    </button>
                </div>
            `;
            card.querySelector(".reprint-btn").addEventListener("click", () => {
    ...
    settingsProfileForm.addEventListener("submit", (e) => {
        ...
        AppDB.saveShopProfile(shopProfile); loadSettingsForm(); alert("Profil toko disimpan!");
    });
    ...
    settingsPaymentForm.addEventListener("submit", (e) => {
        ...
        AppDB.saveShopProfile(shopProfile); loadSettingsForm(); alert("Data pembayaran disimpan!");
    });
    ...
    importDbInput.addEventListener("change", (e) => {
        ...
        reader.onload = function(event) {
            const result = AppDB.importData(event.target.result);
            alert(result.message);
            if (result.success) switchTab(currentTab);
        };
    ...
    btnResetDb.addEventListener("click", () => {
        if (confirm("Hapus seluruh data?")) {
            AppDB.resetAllData(); alert("Data direset."); cart = []; summaryDiscountInput.value = "0"; switchTab("pos");
        }
    });
